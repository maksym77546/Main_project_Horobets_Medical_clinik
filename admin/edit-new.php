            ...
            <form action="update.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-2 mb-4">
                    ...
                    foreach ($days as $num => $name): 
                        $hasSched = isset($sched_map[$num]);
                        $sStart = $hasSched ? substr($sched_map[$num]['start_time'], 0, 5) : '08:00';
                        $sEnd = $hasSched ? substr($sched_map[$num]['end_time'], 0, 5) : '17:00';
                    ?>
                    <div class="col-md-6">
                        <div class="input-group mb-2">
                            <div class="input-group-text bg-light" style="width: 120px;">
                                <input class="form-check-input mt-0 me-2 schedule-check" type="checkbox" name="schedule_days[]" value="<?=$num?>" id="day<?=$num?>" <?=$hasSched?'checked':''?>>
                                <label class="form-check-label" for="day<?=$num?>"><?=$name?></label>
                            </div>
                            <input type="time" name="schedule_start[]" class="form-control" value="<?=$sStart?>" <?=$hasSched?'':'disabled'?>>
                            <span class="input-group-text">-</span>
                            <input type="time" name="schedule_end[]" class="form-control" value="<?=$sEnd?>" <?=$hasSched?'':'disabled'?>>
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
                ...
                <div id="certificates-container">
                    ...
                    <div class="card card-body bg-light mb-3 cert-item">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <h6 class="mb-0">Сертифікат #<?=$idx + 1?></h6>
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-warning flex-grow-1 btn-lg">💾 Оновити дані</button>
...
<script>
    ...
    document.getElementById('add-cert-btn').addEventListener('click', function() {
        ...
        <div class="card card-body bg-light mb-3 cert-item">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <h6 class="mb-0">Новий сертифікат</h6>
                <button type="button" class="btn btn-sm btn-danger remove-cert" title="Видалити">
                    <i class="bi bi-trash"></i>
                </button>
            </div>
